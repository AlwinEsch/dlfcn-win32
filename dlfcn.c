...
void *dlopen( const char *file, int mode )
{
    ...
    else
    {
        ...
        if (wide_len > 0)
        {
          ...
          hModule = LoadPackagedLibrary(lpFileNameW, 0);
...
char *dlerror( void )
{
    ...
    if ( len > 0 )
    {
      int result = WideCharToMultiByte(CP_UTF8, MB_ERR_INVALID_CHARS, messageBuf, len, NULL, 0, NULL, NULL);
      if (result == 0)
        return NULL;

      char* newStr = (char*)malloc(len * sizeof(char));
      result = WideCharToMultiByte(CP_UTF8, MB_ERR_INVALID_CHARS, messageBuf, len, newStr, result, NULL, NULL);
      if (result == 0)
      {
        free(newStr);
        return NULL;
      ...
      ret += copy_string( lpBuffer+ret, sizeof(lpBuffer)-ret, newStr);
      free(newStr);
